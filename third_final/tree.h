...
//                printf("in functiondef\n");
                ...
                while(pnode != NULL)
                {
                    ...
                    else if(strcmp(pnode->name, "IDENTIFIER") == 0) //函数名
                    {
                        ...
                        {
                            printf("line %d: %s has been declared\n", node->line, pnode->str);
                        }
...
//                printf("in variable\n");
                ...
                else if(strcmp(node->pleft->pleft->name, "KCLASS") == 0) //type 为class类型
                {
                    ...
                    {
                        printf("line:%d using class %s in itself is not allowed\n", node->line, pcurclass->name);
                    }
...
//                printf("in simplestmt\n");
                if(strcmp(node->pleft->name, "Call") == 0 && strcmp(node->pleft->pleft->name, "IDENTIFIER") == 0)   //直接函数调用
                {
                    ...
                    else
                    {
                        ...
                        {
                            printf("line:%d parameter num not matched\n", node->line);
                        }
                        else    //checktype
                        {
                            ...
                            if(match == -1)
                            {
                                printf("line:%d some parameter not declrated\n", node->line);
                            }
                            else if(match == 0)
                            {
                                printf("line:%d parameter type not matched\n", node->line);
                            }
                        }
                    }
                }
                else if(strcmp(node->pleft->name, "Call") == 0) //从类中调用函数
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                printf("line:%d no data member named %s in %s\n", node->line, node->pleft->pleft->pright->pright->str, node->pleft->pleft->pleft->pleft->str);
                            }
                            else if(rint == 2)
                            {
                                printf("line:%d no function member named %s in %s\n", node->line, node->pleft->pleft->pright->pright->str, node->pleft->pleft->pleft->pleft->str);
                            }
                            else        //检查参数个数之类的错误
                            {
                                vtype.clear();
                ...
                {
//                    printf("Identifier\n");
                    if(CheckExist(node->pleft->pleft->str) == 0)
                    {
                    ...
                    else
                    {
                        ...
                        if(flag == 1)
                        {
                            printf("line:%d type not matched\n", node->line);
                        }
                    }
                }
//                printf("exit simplestmt\n");
            ...
            else if(strcmp(node->name, "ReturnStmt") == 0)  //用于判断函数返回值
            {
                if(node->pleft->pright == NULL)
                {
                    ...
                    {
                        printf("line:%d return type not matched\n", node->line);
                    }
                }
